...
/**
 * Class that provides easy logging of all the values needed from a motor (controller) to log through
...
public class MotorData {
  ...
  /** The output current of the motor (controller). This is the general output current on a SPARK and the torque current on a Talon. */
  public double outputCurrentAmps = 0.0;
  /** The position reported by the motor controller. Typically motor rotations, but could be different depending on the specific conversion factors on the motor controller. */
  ...
  /** The velocity reported by the motor controller. Typically reported in motor rotations per minute, but could be different depending on the specific conversion factors on the motor controller. */
  public double velocity = 0.0;

  /**
   * Method to log this {@link MotorData} to a {@link LogTable} from an AdvantageKit IO class. Call this inside your IO's {@code toLog()}. Make sure the {@code key} matches the one you call {@code fromLog()} with!
   *
   * @param table The table to log to.
   * @param key The key value to log everything under ("Primary" will log everything under the "Primary/*" key).
   */
  ...
  /**
   * Method to grab the values from a log of this {@link MotorData} from a {@link LogTable} of an AdvantageKit IO class. Call this inside your IO's {@code fromLog()}. Make sure the {@code key} matches the one you call {@code toLog()} with!
   *
   * @param table The table to grab logs from.
   * @param key The key value to grab the logs from ("Primary" will grab everything under the "Primary/*" key).
   */
  public void fromLog(LogTable table, String key) {
    ...
    processorTemperatureCelsius = table.get(key + "/ProcessorTemperatureCelsius", processorTemperatureCelsius);
  ...
  public static class SparkMotorData extends MotorData {
    ...
    /** Debouncer in order to ignore any skipped CAN frames instead of reporting that the controller is disconnected. */
    ...
    /** Updates the motor data using the values from the REV SPARK motor controller linked with this instance. */
    public void update() {
      // Reset the sticky fault status from SparkUtil
      ...
      motorTemperatureCelsius = SparkUtil.ifOkOrDefault(spark, spark::getMotorTemperature, motorTemperatureCelsius);
      ...
      connected = connectedDebouncer.calculate(!SparkUtil.sparkStickyFault);
    }
  }
}
